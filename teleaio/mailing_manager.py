...
class MailingManager:
    def __init__(self, bot: Bot, db: Database):
        ...
        self._send_semaphore = asyncio.Semaphore(10)  # Ограничение одновременных отправок
    ...
    async def _process_mailing_loop(self, mailing_id):
        """Параллельная рассылка с защитой от блокировок"""
        try:
            while self.running and mailing_id in self.active_mailings:
                ...
                # Проверяем лимиты для всех аккаунтов перед отправкой
                ...
                # Случайное распределение целей по аккаунтам
                random.shuffle(accounts_ok)
                tasks = []
                ...
                # Отправляем все сообщения параллельно с ограничением
                results = []
                for task in tasks:
                ...
                if mailing_id in self.active_mailings:
                    ...
                    mailing['current_cycle'] += 1
                    await self.db.update_mailing_status(mailing_id, 'running', mailing['sent_count'])
                
                logger.info(f"✅ Цикл {mailing.get('current_cycle', 0)}: +{sent_count} успешно, {failed_count} ошибок")
